...
{
	if (!(hw && hw->adpt && hw->ALX_Adapter.pdev))
		return -EINVAL;
	return pci_read_config_word(hw->ALX_Adapter.pdev, reg, pval);
}
...
int alx_cfg_w16(const struct alx_hw *hw, int reg, u16 val)
{
	if (!(hw && hw->adpt && hw->ALX_Adapter.pdev))
		return -EINVAL;
	return pci_write_config_word(hw->ALX_Adapter.pdev, reg, val);
}
		   ...
		   const char *fmt, ...)
{
	...
	if (hw && hw->adpt && hw->ALX_Adapter.netdev)
		__netdev_printk(level, hw->ALX_Adapter.netdev, &vaf);
	else
...
static int alx_set_mac_type(struct alx_adapter *adpt)
{
	...
	netif_info(adpt, hw, ALX_Adapter.netdev,
...
static void alx_task_schedule(struct alx_adapter *adpt)
{
	if (!CHK_ADPT_FLAG(STATE_DOWN) &&
		...
		schedule_work(&ALX_Adapter.alx_task);
	}
}
...
static void alx_check_lsc(struct alx_adapter *adpt)
{
	...
	ALX_Adapter.link_jiffies = jiffies + ALX_TRY_LINK_TIMEOUT;
...
static int alx_refresh_rx_buffer(struct alx_rx_queue *rxque)
{
	...
	while (next_rxbuf->dma == 0) {
		skb = dev_alloc_skb(ALX_Adapter.rxbuf_size);
		if (unlikely(!skb)) {
	...
	if (count) {
		...
		netif_info(adpt, rx_err, ALX_Adapter.netdev,
				...
				struct alx_rx_queue *rxque)
{
	...
	struct pci_dev *pdev = ALX_Adapter.pdev;
	struct net_device *netdev  = ALX_Adapter.netdev;
	...
	while (1) {
		...
		if (srrd.genr.res || srrd.genr.lene) {
			...
			netif_warn(adpt, rx_err, ALX_Adapter.netdev,
		...
		/* start to dispatch */
		swque = ALX_Adapter.rx_queue[srrd.genr.rss_cpu];
			      ...
			      struct alx_tx_queue *txque)
{
	...
	netif_info(adpt, tx_err, ALX_Adapter.netdev,
	...
	while (txque->tpq.consume_idx != consume_data) {
		...
		if (tpbuf->dma) {
			pci_unmap_page(ALX_Adapter.pdev, tpbuf->dma, tpbuf->length,
				       DMA_TO_DEVICE);
			tpbuf->dma = 0;
		}
	...
	if (netif_queue_stopped(ALX_Adapter.netdev) &&
		netif_carrier_ok(ALX_Adapter.netdev)) {
		netif_wake_queue(ALX_Adapter.netdev);
	}
	return true;
}
...
{
	struct alx_msix_param *msix = (struct alx_msix_param *)data;
	struct alx_adapter *adpt = msix->adpt;
	struct alx_hw *hw = &ALX_Adapter.hw;
	u32 isr;
	...
	if (isr & ALX_ISR_MANU) {
		ALX_Adapter.net_stats.tx_carrier_errors++;
		alx_check_lsc(adpt);
	}
...
static irqreturn_t alx_msix_rtx(int irq, void *data)
{
	...
	netif_info(adpt, intr, ALX_Adapter.netdev,
		   "msix vec_idx = %d\n", msix->vec_idx);
...
static int alx_napi_msix_rtx(struct napi_struct *napi, int max_pkts)
{
	...
	for (i = 0; i < msix->rx_count; i++) {
		...
		num_pkts = 0;
		if (CHK_ADPT_FLAG(SRSS_EN)) {
			if (!spin_trylock_irqsave(&ALX_Adapter.rx_lock, flags))
				goto clean_sw_irq;

			for (j = 0; j < ALX_Adapter.num_hw_rxques; j++)
				alx_dispatch_rx_irq(msix, ALX_Adapter.rx_queue[j]);

			spin_unlock_irqrestore(&ALX_Adapter.rx_lock, flags);
clean_sw_irq:
			swque = ALX_Adapter.rx_queue[rque_idx];
			complete &= alx_handle_srx_irq(msix, swque, &num_pkts,
						       max_pkts);

		} else {
			rxque = ALX_Adapter.rx_queue[rque_idx];
	...
	for (i = 0; i < msix->tx_count; i++) {
		...
		txque = ALX_Adapter.tx_queue[tque_idx];
...
static int alx_napi_legacy_rtx(struct napi_struct *napi, int max_pkts)
{
	...
	if (!netif_carrier_ok(ALX_Adapter.netdev))
		goto enable_rtx_irq;

	for (que_idx = 0; que_idx < ALX_Adapter.num_txques; que_idx++)
		complete &= alx_handle_tx_irq(msix, ALX_Adapter.tx_queue[que_idx]);

	for (que_idx = 0; que_idx < ALX_Adapter.num_hw_rxques; que_idx++) {
		num_pkts = 0;
		complete &= alx_handle_rx_irq(msix, ALX_Adapter.rx_queue[que_idx],
					      &num_pkts, max_pkts);
	}
...
static int alx_setup_msix_maps(struct alx_adapter *adpt)
{
	int msix_idx = 0;
	int que_idx = 0;
	int num_rxques = ALX_Adapter.num_rxques;
	int num_txques = ALX_Adapter.num_txques;
	int num_msix_rxques = ALX_Adapter.num_msix_rxques;
	int num_msix_txques = ALX_Adapter.num_msix_txques;
	int num_msix_noques = ALX_Adapter.num_msix_noques;

	if (CHK_ADPT_FLAG(FIXED_MSIX))
		goto fixed_msix_map;

	netif_warn(adpt, ifup, ALX_Adapter.netdev,
...
fixed_msix_map:
	...
	for (que_idx = 0; que_idx < num_msix_rxques; que_idx++, msix_idx++) {
		struct alx_msix_param *msix = ALX_Adapter.msix[msix_idx];
		if (que_idx < num_rxques) {
			ALX_Adapter.rx_queue[que_idx]->msix = msix;
	...
	if (msix_idx != num_msix_rxques)
		netif_warn(adpt, ifup, ALX_Adapter.netdev, "msix_idx is wrong\n");
	...
	for (que_idx = 0; que_idx < num_msix_txques; que_idx++, msix_idx++) {
		...
		if (que_idx < num_txques) {
			ALX_Adapter.tx_queue[que_idx]->msix = msix;
...
static inline void alx_reset_msix_maps(struct alx_adapter *adpt)
{
	...
	for (que_idx = 0; que_idx < ALX_Adapter.num_rxques; que_idx++)
		ALX_Adapter.rx_queue[que_idx]->msix = NULL;
	for (que_idx = 0; que_idx < ALX_Adapter.num_txques; que_idx++)
		ALX_Adapter.tx_queue[que_idx]->msix = NULL;

	for (msix_idx = 0; msix_idx < ALX_Adapter.num_msix_intrs; msix_idx++) {
		struct alx_msix_param *msix = ALX_Adapter.msix[msix_idx];
...
static irqreturn_t alx_interrupt(int irq, void *data)
{
	...
	struct alx_adapter *adpt = netdev_priv(netdev);
	struct alx_hw *hw = &ALX_Adapter.hw;
	struct alx_msix_param *msix = ALX_Adapter.msix[0];
	...
	do {
		...
		if (status & ALX_ISR_ERROR) {
			netif_warn(adpt, intr, ALX_Adapter.netdev,
...
static int alx_request_msix_irq(struct alx_adapter *adpt)
{
	...
	int msix_idx;
	int num_msix_intrs = ALX_Adapter.num_msix_intrs;
	...
	for (msix_idx = 0; msix_idx < num_msix_intrs; msix_idx++) {
		...
		if (CHK_MSIX_FLAG(RXS) && CHK_MSIX_FLAG(TXS)) {
			...
			netif_info(adpt, ifup, ALX_Adapter.netdev,
		...
		retval = request_irq(ALX_Adapter.msix_entries[msix_idx].vector,
				     handler, 0, msix->name, msix);
		if (retval)
			goto free_msix_irq;

		/* assign the mask for this irq */
		irq_set_affinity_hint(ALX_Adapter.msix_entries[msix_idx].vector,
				      msix->affinity_mask);
	}
	return retval;
...
free_msix_irq:
	for (i = 0; i < msix_idx; i++) {
		irq_set_affinity_hint(ALX_Adapter.msix_entries[i].vector, NULL);
		free_irq(ALX_Adapter.msix_entries[i].vector, ALX_Adapter.msix[i]);
	}
	CLI_ADPT_FLAG(0, MSIX_EN);
	pci_disable_msix(ALX_Adapter.pdev);
	kfree(ALX_Adapter.msix_entries);
	ALX_Adapter.msix_entries = NULL;
	return retval;
}
...
static int alx_request_irq(struct alx_adapter *adpt)
{
	...
	if (CHK_ADPT_FLAG(MSI_EN)) {
		retval = request_irq(ALX_Adapter.pdev->irq, alx_interrupt, 0,
			netdev->name, netdev);
		if (retval) {
			alx_err(adpt, "request msix irq failed, error = %d\n",
				retval);
	...
	/* request shared irq */
	retval = request_irq(ALX_Adapter.pdev->irq, alx_interrupt, IRQF_SHARED,
			netdev->name, netdev);
	if (retval) {
...
static void alx_free_irq(struct alx_adapter *adpt)
{
	...
	if (CHK_ADPT_FLAG(MSIX_EN)) {
		for (i = 0; i < ALX_Adapter.num_msix_intrs; i++) {
			struct alx_msix_param *msix = ALX_Adapter.msix[i];
			netif_info(adpt, ifdown, ALX_Adapter.netdev,
				   "msix entry = %d\n", i);
			if (!CHK_MSIX_FLAG(ALL))
				continue;
			if (CHK_MSIX_FLAG(RXS) || CHK_MSIX_FLAG(TXS)) {
				irq_set_affinity_hint(
					ALX_Adapter.msix_entries[i].vector, NULL);
			}
			free_irq(ALX_Adapter.msix_entries[i].vector, msix);
		}
		alx_reset_msix_maps(adpt);
	} else {
		free_irq(ALX_Adapter.pdev->irq, netdev);
	}
}
...
{
	alx_vlan_mode(ALX_Adapter.netdev, ALX_Adapter.netdev->features);
}
...
static void alx_napi_enable_all(struct alx_adapter *adpt)
{
	...
	for (msix_idx = 0; msix_idx < num_msix_intrs; msix_idx++) {
		...
		msix = ALX_Adapter.msix[msix_idx];
		napi = &msix->napi;
		napi_enable(napi);
	}
}
...
static void alx_clean_all_tx_queues(struct alx_adapter *adpt)
{
	...
	for (i = 0; i < ALX_Adapter.num_txques; i++)
		alx_clean_tx_queue(ALX_Adapter.tx_queue[i]);
}
...
static void alx_clean_all_rx_queues(struct alx_adapter *adpt)
{
	...
	for (i = 0; i < ALX_Adapter.num_rxques; i++)
		alx_clean_rx_queue(ALX_Adapter.rx_queue[i]);
}
...
static inline void alx_set_num_rxques(struct alx_adapter *adpt)
{
	if (CHK_ADPT_FLAG(SRSS_CAP)) {
		...
		ALX_Adapter.num_rxques =
			max_t(u16, ALX_Adapter.num_hw_rxques, ALX_Adapter.num_sw_rxques);
	}
}
...
static int alx_alloc_all_rtx_queue(struct alx_adapter *adpt)
{
	...
	for (que_idx = 0; que_idx < ALX_Adapter.num_txques; que_idx++) {
		...
		txque->dev = &ALX_Adapter.pdev->dev;
		txque->netdev = ALX_Adapter.netdev;
	...
	for (que_idx = 0; que_idx < ALX_Adapter.num_rxques; que_idx++) {
		...
		rxque->dev = &ALX_Adapter.pdev->dev;
		rxque->netdev = ALX_Adapter.netdev;
	...
	netif_dbg(adpt, probe, ALX_Adapter.netdev,
...
static int alx_set_interrupt_param(struct alx_adapter *adpt)
{
	...
	if (CHK_ADPT_FLAG(MSIX_EN)) {
		...
		ALX_Adapter.num_msix_intrs = 1;
		poll = &alx_napi_legacy_rtx;
	}

	for (msix_idx = 0; msix_idx < ALX_Adapter.num_msix_intrs; msix_idx++) {
		msix = (struct alx_msix_param *)kzalloc(sizeof(struct alx_msix_param),
                                            GFP_KERNEL);
		if (!msix)
			goto err_alloc_msix;
		msix->adpt = adpt;
		...
		netif_napi_add(ALX_Adapter.netdev, &msix->napi, (*poll), 64);
		ALX_Adapter.msix[msix_idx] = msix;
	}
	return 0;

err_alloc_cpumask:
	kfree(msix);
	ALX_Adapter.msix[msix_idx] = NULL;
err_alloc_msix:
...
static int alx_set_msix_interrupt_mode(struct alx_adapter *adpt)
{
	...
	ALX_Adapter.msix_entries = (struct msix_entry *)kcalloc(ALX_Adapter.max_msix_intrs,
				sizeof(struct msix_entry), GFP_KERNEL);
	if (!ALX_Adapter.msix_entries) {
		netif_info(adpt, probe, ALX_Adapter.netdev,
			   "can't allocate msix entry\n");
		CLI_ADPT_FLAG(0, MSIX_EN);
		goto try_msi_mode;
	}

	for (msix_idx = 0; msix_idx < ALX_Adapter.max_msix_intrs; msix_idx++)
		ALX_Adapter.msix_entries[msix_idx].entry = msix_idx;


	msix_intrs = ALX_Adapter.max_msix_intrs;
	while (msix_intrs >= ALX_Adapter.min_msix_intrs) {
		retval = pci_enable_msix(ALX_Adapter.pdev, ALX_Adapter.msix_entries,
				      msix_intrs);
	...
	if (msix_intrs < ALX_Adapter.min_msix_intrs) {
		netif_info(adpt, probe, ALX_Adapter.netdev,
	...
	ALX_Adapter.num_msix_intrs = min_t(int, msix_intrs, ALX_Adapter.max_msix_intrs);
	retval = 0;
	return retval;
...
static int alx_set_msi_interrupt_mode(struct alx_adapter *adpt)
{
	...
	retval = pci_enable_msi(ALX_Adapter.pdev);
	if (retval) {
		netif_info(adpt, probe, ALX_Adapter.netdev,
...
static void alx_reset_interrupt_mode(struct alx_adapter *adpt)
{
	if (CHK_ADPT_FLAG(MSIX_EN)) {
		...
		pci_disable_msi(ALX_Adapter.pdev);
	}
}
...
static int __devinit alx_init_adapter(struct alx_adapter *adpt)
{
	...
	ALX_Adapter.max_msix_intrs = 1;
	ALX_Adapter.min_msix_intrs = 1;
	max_frame = ALX_Adapter.netdev->mtu + ETH_HLEN + ETH_FCS_LEN + VLAN_HLEN;
	ALX_Adapter.rxbuf_size = ALX_Adapter.netdev->mtu > ALX_DEF_RX_BUF_SIZE ?
				   ...
				   struct alx_tx_queue *txque)
{
	...
	txque->tpq.tpdesc = (union alx_hw_tpdesc *)((char *)ring_header->desc +
                                           ring_header->used);
	ALX_Adapter.hw.tpdma[txque->que_idx] = (u64)txque->tpq.tpdma;
...
static int alx_alloc_all_tx_descriptor(struct alx_adapter *adpt)
{
	...
	for (i = 0; i < ALX_Adapter.num_txques; i++) {
		retval = alx_alloc_tx_descriptor(adpt, ALX_Adapter.tx_queue[i]);
		if (!retval)
			continue;
				   ...
				   struct alx_rx_queue *rxque)
{
	...
	if (CHK_RX_FLAG(HW_QUE)) {
		...
		rxque->rrq.rrdesc = (union alx_hw_rrdesc *)((char *)ring_header->desc +
                                             ring_header->used);
		ALX_Adapter.hw.rrdma[rxque->que_idx] = (u64)rxque->rrq.rrdma;
		...
		rxque->rfq.rfdesc = (union alx_hw_rfdesc *)((char *)ring_header->desc +
                                             ring_header->used);
		ALX_Adapter.hw.rfdma[rxque->que_idx] = (u64)rxque->rfq.rfdma;
...
static int alx_alloc_all_rx_descriptor(struct alx_adapter *adpt)
{
	...
	for (i = 0; i < ALX_Adapter.num_rxques; i++) {
		error = alx_alloc_rx_descriptor(adpt, ALX_Adapter.rx_queue[i]);
		if (!error)
			continue;
...
static void alx_free_all_tx_descriptor(struct alx_adapter *adpt)
{
	...
	for (i = 0; i < ALX_Adapter.num_txques; i++)
		alx_free_tx_descriptor(ALX_Adapter.tx_queue[i]);
}
...
static void alx_free_all_rx_descriptor(struct alx_adapter *adpt)
{
	int i;
	for (i = 0; i < ALX_Adapter.num_rxques; i++)
		alx_free_rx_descriptor(ALX_Adapter.rx_queue[i]);
}
...
{
	struct device *dev = &ALX_Adapter.pdev->dev;
	struct alx_ring_header *ring_header = &ALX_Adapter.ring_header;
	...
	ALX_Adapter.cmb.dma = ring_header->dma + ring_header->used;
	ALX_Adapter.cmb.cmb = (u8 *) ring_header->desc + ring_header->used;
	ring_header->used += ALIGN(sizeof(struct coals_msg_block), 8);

	ALX_Adapter.smb.dma = ring_header->dma + ring_header->used;
	ALX_Adapter.smb.smb = (u8 *)ring_header->desc + ring_header->used;
...
static void alx_free_all_rtx_descriptor(struct alx_adapter *adpt)
{
	...
	ALX_Adapter.cmb.dma = 0;
	ALX_Adapter.cmb.cmb = NULL;
	ALX_Adapter.smb.dma = 0;
	ALX_Adapter.smb.smb = NULL;
					  ...
					  netdev_features_t features)
{
	...
	if (netdev->mtu > ALX_MAX_TSO_PKT_SIZE ||
	    ALX_Adapter.hw.mac_type == alx_mac_l1c ||
	    ALX_Adapter.hw.mac_type == alx_mac_l2c)
...
static int alx_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	if (old_mtu != new_mtu && netif_running(netdev)) {
		netif_info(adpt, hw, ALX_Adapter.netdev,
			   ...
			   netdev->mtu, new_mtu);
		netdev->mtu = new_mtu;
		ALX_Adapter.hw.mtu = new_mtu;
		ALX_Adapter.rxbuf_size = new_mtu > ALX_DEF_RX_BUF_SIZE ?
				   ALIGN(max_frame, 8) : ALX_DEF_RX_BUF_SIZE;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,39))
		if (new_mtu > (7*1024)) {
...
static int alx_open_internal(struct alx_adapter *adpt, u32 ctrl)
{
	...
	alx_set_multicase_list(ALX_Adapter.netdev);
	alx_restore_vlan(adpt);

	if (hw->cbs.config_mac)
		retval = hw->cbs.config_mac(hw, ALX_Adapter.rxbuf_size,
	...
	for (i = 0; i < ALX_Adapter.num_hw_rxques; i++)
		alx_refresh_rx_buffer(ALX_Adapter.rx_queue[i]);
	...
	netif_tx_start_all_queues(ALX_Adapter.netdev);
	...
	SET_ADPT_FLAG(TASK_LSC_REQ);
	ALX_Adapter.link_jiffies = jiffies + ALX_TRY_LINK_TIMEOUT;
	mod_timer(&ALX_Adapter.alx_timer, jiffies);
...
static void alx_stop_internal(struct alx_adapter *adpt, u32 ctrl)
{
	...
	del_timer_sync(&ALX_Adapter.alx_timer);
	...
	ALX_Adapter.hw.link_speed = ALX_LINK_SPEED_UNKNOWN;
...
static int alx_stop(struct net_device *netdev)
{
	...
	if (CHK_ADPT_FLAG(STATE_RESETTING))
		netif_warn(adpt, ifdown, ALX_Adapter.netdev,
...
{
	struct alx_adapter *adpt = (struct alx_adapter *)pci_get_drvdata(pdev);
	struct net_device *netdev = ALX_Adapter.netdev;
	struct alx_hw *hw = &ALX_Adapter.hw;
...
static void alx_update_hw_stats(struct alx_adapter *adpt)
{
	...
	struct alx_hw *hw = &ALX_Adapter.hw;
	struct alx_hw_stats *hwstats = &ALX_Adapter.hw_stats;
	...
	net_stats = &ALX_Adapter.netdev->stats;
...
static void alx_link_task_routine(struct alx_adapter *adpt)
{
	...
	netif_info(adpt, timer, ALX_Adapter.netdev,
		   "link_speed = %d, link_up = %d\n",
		   hw->link_speed, hw->link_up);

	if (!hw->link_up && time_after(ALX_Adapter.link_jiffies, jiffies))
		SET_ADPT_FLAG(TASK_LSC_REQ);
	...
	if (hw->link_up) {
		...
		hw->link_speed = 0;
		netif_info(adpt, timer, ALX_Adapter.netdev, "NIC Link is Down\n");
		netif_carrier_off(netdev);
		netif_tx_stop_all_queues(netdev);
...
static void alx_timer_routine(unsigned long data)
{
	...
	/* Reset the timer */
	mod_timer(&ALX_Adapter.alx_timer, delay + jiffies);
...
static void alx_task_routine(struct work_struct *work)
{
	...
	if (!CHK_ADPT_FLAG(STATE_WATCH_DOG))
		netif_warn(adpt, timer, ALX_Adapter.netdev,
					...
					struct sk_buff *skb)
{
	...
	if (!alx_check_num_tpdescs(txque, skb)) {
		...
		netif_stop_queue(ALX_Adapter.netdev);
			 ...
			 struct ifreq *ifr, int cmd)
{
	...
	case SIOCGMIIREG:
		...
		retval = hw->cbs.read_phy_reg(hw, data->reg_num,
					      &data->val_out);
		netif_dbg(adpt, hw, ALX_Adapter.netdev, "read phy %02x %04x\n",
	...
	case SIOCSMIIREG:
		...
		netif_dbg(adpt, hw, ALX_Adapter.netdev, "write phy %02x %04x\n",
...
static void alx_poll_controller(struct net_device *netdev)
{
	...
	if (CHK_ADPT_FLAG(MSIX_EN)) {
		...
		alx_interrupt(ALX_Adapter.pdev->irq, netdev);
	}
}
#endif
		       ...
		       const struct pci_device_id *ent)
{
	...
	ALX_Adapter.netdev = netdev;
	ALX_Adapter.pdev = pdev;
	hw = &ALX_Adapter.hw;
	hw->adpt = adpt;
	ALX_Adapter.msg_enable = ALX_MSG_DEFAULT;

	ALX_Adapter.hw.hw_addr = (u8 *)ioremap(pci_resource_start(pdev, BAR_0),
				   pci_resource_len(pdev, BAR_0));
	if (!ALX_Adapter.hw.hw_addr) {
		alx_err(adpt, "cannot map device registers\n");
		retval = -EIO;
		goto err_iomap;
	}
	netdev->base_addr = (unsigned long)ALX_Adapter.hw.hw_addr;
	...
	ALX_Adapter.bd_number = cards_found;

	// Initialise alx_adapter structure.
	if (ALX_initAdapter() != kIOReturnSuccess)
  {
	...
	/* get user settings */
	ALX_Adapter.num_txdescs = 1024;
	ALX_Adapter.num_rxdescs = 512;
	...
	netdev->hw_features = NETIF_F_SG	 |
			      NETIF_F_HW_CSUM	 |
			      NETIF_F_HW_VLAN_RX;
	if (ALX_Adapter.hw.mac_type != alx_mac_l1c &&
	    ALX_Adapter.hw.mac_type != alx_mac_l2c) {
	...
	setup_timer(&ALX_Adapter.alx_timer, &alx_timer_routine,
		    (unsigned long)adpt);
	INIT_WORK(&ALX_Adapter.alx_task, alx_task_routine);
	...
	netif_dbg(adpt, probe, ALX_Adapter.netdev,
		  ...
		  ALX_Adapter.num_msix_noques, ALX_Adapter.num_msix_rxques,
		  ALX_Adapter.num_msix_txques);
	netif_dbg(adpt, probe, ALX_Adapter.netdev, "num_msix_all_intrs = %d\n",
		  ALX_Adapter.num_msix_intrs);

	netif_dbg(adpt, probe, ALX_Adapter.netdev,
		  "RX Queue Count = %u, HRX Queue Count = %u, "
		  "SRX Queue Count = %u, TX Queue Count = %u\n",
		  ALX_Adapter.num_rxques, ALX_Adapter.num_hw_rxques, ALX_Adapter.num_sw_rxques,
		  ALX_Adapter.num_txques);
	...
	device_set_wakeup_enable(&ALX_Adapter.pdev->dev, ALX_Adapter.wol);
	...
	ALX_Adapter.netdev_registered = true;
	...
	netif_info(adpt, probe, ALX_Adapter.netdev, "%pM\n", netdev->dev_addr);
...
err_init_adapter:
	iounmap(ALX_Adapter.hw.hw_addr);
err_iomap:
	free_netdev(netdev);
err_alloc_netdev:
...
static void __devexit alx_remove(struct pci_dev *pdev)
{
	...
	SET_ADPT_FLAG(STATE_DOWN);
	cancel_work_sync(&ALX_Adapter.alx_task);
	...
	if (ALX_Adapter.netdev_registered) {
		unregister_netdev(netdev);
		ALX_Adapter.netdev_registered = false;
	}
	...
	netif_info(adpt, probe, ALX_Adapter.netdev, "complete\n");
	free_netdev(netdev);
